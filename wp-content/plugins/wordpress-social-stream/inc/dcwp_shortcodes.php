...
//	print_r($atts);
			...
			foreach($defaults[$function] as $k=>$v){
				...
				if($k == 'out'){
					...
					if(isset($stream['feeds_'.$function.'_'.$k])){
						if($function.'_'.$k == 'twitter_thumb' || $function.'_'.$k == 'twitter_retweets' || $function.'_'.$k == 'twitter_replies' || $function.'_'.$k == 'facebook_comments' || $function.'_'.$k == 'facebook_image_width'){
	...
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.dc_jqsocialstream::get_plugin_directory().'/js/jquery.social.stream.1.5.4.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
...
//	print_r($atts);
	...
	$out .='<script type="text/javascript">jQuery(document).ready(function($){';
	$out .= 'var config = '.$config.';';
	$out .= 'if(!jQuery().dcSocialStream) { $.getScript("'.dc_jqsocialstream::get_plugin_directory().'/js/jquery.social.stream.wall.1.3.js", function(){}); $.getScript("'.dc_jqsocialstream::get_plugin_directory().'/js/jquery.social.stream.1.5.4.min.js", function(){$("#social-stream-'.$id.'").dcSocialStream(config);}); } else {';
